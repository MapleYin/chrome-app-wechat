...
export class WxLogin{
	...
	// 获取UUID
	private getUUID(resolve,reject){
		let self = this;
	...
	// 得到 二维码
	private getQRImage(UUID,resolve,reject){
		let self = this;
		let timestamp = new Date();
		$.ajax({
	...
	// 等待扫描与确认登录
	private waitForScan(UUID:string,didScan:boolean,resolve,reject){
		let self = this;
		let timestamp = new Date();
		$.ajax({
			url : WAITLOGIN_URL,
	...
	// 初始化得到cookie
	private getCookies(url:URL,resolve,reject){
		let self = this;
		let timestamp = new Date();
		$.ajax({
			url : LOGIN_URL,
	...
	private convertXMLToJSON(XMLString):any{
		var json = {},
			result;
